...
TOOLCHAIN_TARGET_TASK = "${TOOLCHAIN_TARGET_GMAETASK} task-qte-toolchain-target"
TOOLCHAIN_OUTPUTNAME = "${SDK_NAME}-toolchain-sdk-${SDK_VERSION}-${SDK_SUBVERSION}${EXTRAVERSION}"
...
toolchain_create_sdk_version_append() {
	...
	echo 'SDK Version: ${SDK_VERSION}-${SDK_SUBVERSION}${EXTRAVERSION}' >> $versionfile
	echo 'Timestamp: ${DATETIME}' >> $versionfile
}

# echo 'Metadata Revision: ${METADATA_BRANCH}:${METADATA_REVISION}' >> $versionfile
